class UsersController < ApplicationController
  ...
  before_action :filtered_users, only: [ :index, :homepage, :show ]
  ...
  def map
  current_user.geocode if current_user.latitude.blank?

  radius = params[:radius].present? ? params[:radius].to_i : 10

  search_lat = params[:latitude].present? ? params[:latitude].to_f : current_user.latitude
  search_lng = params[:longitude].present? ? params[:longitude].to_f : current_user.longitude
  ...
  current_user.received_matches.each do |match|
    match_users << match.user_requester_id
  end

  current_user.requested_matches.each do |match|
    match_users << match.user_receiver_id
  end

  users = users.reject { |u| match_users.include?(u.id) }

  # Activity filter
  current_activities = current_user.activities.pluck(:name)

  unless current_activities.empty?
    users = users.select do |user|
      (user.activities.pluck(:name) & current_activities).any?
    end
  end

  # Fitness filter
  if current_user.level_of_fitness.present?
    users = users.select do |user|
      user.level_of_fitness == current_user.level_of_fitness
    end
  end
  ...
  users = users.select do |user|
    if current_user.partner_gender_preference == "Any"
      user.partner_gender_preference == current_user.gender ||
        user.partner_gender_preference == "Any"
    else
      user.gender == current_user.partner_gender_preference &&
      (user.partner_gender_preference == current_user.gender ||
       user.partner_gender_preference == "Any")
    end
  end
  ...
  # Distance filter
  @users_matching = users.select do |user|
    next false unless user.latitude.present? && user.longitude.present?
...
@users_matching.each do |user|
  puts "#{user.first_name} -> #{user.latitude}, #{user.longitude}"
end
